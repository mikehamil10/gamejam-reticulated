...
key_suck = keyboard_check(vk_lcontrol);
key_blow = keyboard_check(vk_lshift);
// this is only used once, but just for consistency's sake:
key_next_room = keyboard_check_pressed(vk_escape);
...
// No. Handle movement
    if (key_left || (joystick_exists(1) && joystick_direction(1) == vk_numpad4)) {
        ...
        image_xscale = 1;
        self.spriteMode = 2;
    } else if (key_right || (joystick_exists(1) && joystick_direction(1) == vk_numpad6)) {
        phy_position_x += self.speedX;
        if(self.isJumping){
            sprite_index = sprAvatarIdle;
        } else {
    ...
    if (!self.isJumping && (key_jump || (joystick_exists(1) && joystick_check_button(1, 1)))) {
        self.isJumping = true;
    ...
    if ((key_suck && key_down) || (joystick_exists(1) && joystick_check_button(1, 3) && joystick_direction(1) == vk_numpad2)){
        self.suckDirection = "down";
        isSucking = true;
        sprite_index = sprAvatarSuck;
        self.spriteMode = 4;
    } else if ((key_suck && key_up) || (joystick_exists(1) && joystick_check_button(1, 3) && joystick_direction(1) == vk_numpad8)){
        self.suckDirection = "up";
        isSucking = true;
        sprite_index = sprAvatarSuck;
        self.spriteMode = 4;
    } else if (key_suck || (joystick_exists(1) && joystick_check_button(1, 3))) {
        self.suckDirection = "side";
        isSucking = true;
        sprite_index = sprAvatarSuck;
        self.spriteMode = 4;
    } else if (key_blow || (joystick_exists(1) && joystick_check_button(1, 2))) { //TODO: controller equivalent
        isSucking = true;
        sprite_index = sprAvatarSuck;
        self.spriteMode = 5;
    }
